...
class enemy:
    ...
    def bring_enemy(self, tmr): #敵を出す
        sec = tmr/30
        if 0 < sec and sec < 15 and tmr%60 == 0:
            self.set_enemy(random.randint(20, 940), LINE_T, 90, EMY_ZAKO, 8, 1) #敵1
            self.set_enemy(random.randint(20, 940), LINE_T, 90, EMY_ZAKO+1, 12, 1) #敵2
            self.set_enemy(random.randint(100, 860), LINE_T, random.randint(60, 120), EMY_ZAKO+2, 6, 3) #敵3
            self.set_enemy(random.randint(100, 860), LINE_T, 90, EMY_ZAKO+3, 12, 2) #敵4
        if tmr == 30*20: #ボス出現
            self.set_enemy(480, -210, 90, EMY_BOSS, 4, 200)
    ...
    def move_enemy(self ,scrn ,msl:missile ,eff:effect, shield:shield, score:score, tmr): #敵機の移動
            ...
            if self.emy_f[i] == True:
                ...
                if self.emy_type[i] < EMY_BOSS: #ザコの動き
                    ...
                    if self.emy_type[i] == 4: #進行方向を変える敵
                        ...
                        if self.emy_y[i] > 240 and self.emy_a[i] == 90:
                            self.emy_a[i] = random.choice([50,70,110,130])
                            self.set_enemy(self.emy_x[i], self.emy_y[i], 90, EMY_BULLET, 6, 0)
                    if self.emy_x[i] < LINE_L or LINE_R < self.emy_x[i] or self.emy_y[i] < LINE_T or LINE_B < self.emy_y[i]:
                        self.emy_f[i] = False
                else: #ボスの動き
                    if self.emy_count[i] == 0:
                        self.emy_y[i] = self.emy_y[i] + 2
                        if self.emy_y[i] >= 200:
                            self.emy_count[i] = 1
                    elif self.emy_count[i] == 1:
                        self.emy_x[i] = self.emy_x[i] - self.emy_speed[i]
                        if self.emy_x[i] < 200:
                            for j in range(0, 10):
                                self.set_enemy(self.emy_x[i], self.emy_y[i]+80, j*20, EMY_BULLET, 6, 0)
                            self.emy_count[i] = 2
                    else:
                        self.emy_x[i] = self.emy_x[i] + self.emy_speed[i]
                        if self.emy_x[i] > 760:
                        ...
                        if self.emy_shield[i] < 100 and tmr%30 == 0:
                            self.set_enemy(self.emy_x[i], self.emy_y[i]+80, random.randint(60, 120), EMY_BULLET, 6, 0)
                ...
                if self.emy_type[i] != EMY_BULLET: #プレイヤーと玉とのヒットチェック
                    ...
                    if msl.has_hit_enemy(self.emy_x[i], self.emy_y[i], r):
                        eff.set_effect(self.emy_x[i]+random.randint(-er, er), self.emy_y[i]+random.randint(-er, er))
                        if self.emy_type[i] == EMY_BOSS: #ボスはフラッシュさせる
                            png = self.emy_type[i] + 1
...
def main(): #メインループ
    ...
    while True:
        ...
        if idx == 1: #ゲームプレイ中
            ...
            emy.bring_enemy(tmr)
            emy.move_enemy(screen,msl,eff,sld,scr,tmr)
            if tmr == 30*60:
                idx = 3
                tmr = 0
